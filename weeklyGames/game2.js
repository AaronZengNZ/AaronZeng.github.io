...
function fightTick(){
    ...
    fightStats[4].innerHTML = "SIZE "+fightStats[3].opSize+" LVL "+fightStats[3].opLvl+" "+fightStats[3].opType+'|HP:'+fightStats[3].opHp+'|DMG:'+fightStats[3].opDmg+'|DEF:'+fightStats[3].opDef+'|LVL:'+fightStats[3].opLvl
...
window.setInterval(function(){
    if(fighting == true){
        if(fightStats[3].opHp > 0){
            ...
            fighting = false;
            fightStats[4].innerHTML = 'WIN! +'+fightStats[3].opSize+'g'
            gold += fightStats[3].opSize;
            fightStats[2].innerHTML = gold+' GOLD'
        }
    }
},100)
fightStats[0].onclick = () => {
    ...
    fightStats[3].opSize = (Math.floor((emStats-50)/20)+1)*Math.floor(Math.random()*3)
    console.log(Math.floor(20 % emStats-10))
    fightStats[3].opType = enemies[Math.floor((emStats-50)/20)]
    fightStats[3].opLvl = Math.floor((emStats - 40)/Math.floor(Math.random()*5+5))
...
window.setInterval(function() {
    if(humanAlreadyExists){
        ...
        human[0].innerHTML = 'Dmg:'+humDmg+'|Hp:'+humHp+'|Def:'+humDef+"|Level:"+humLev+"|Rank:"+rank
    }
},5000)
